...
function main(n=10)
    ...
    f_out = open("../data/two_assortments_n_10_K_10.csv", "w")
    ...
    for iter=1:num_iterations, K in Ks
        ...
        for i1_temp=union(S1_minus_S2,Set(Inf)),i2_temp=union(S2_minus_S1,Set(Inf))
            ...
            robust_revenue = getobjectivevalue(model) + sum(ρ[i,i]*v[1][i] for i in S1_intersect_S2)
            ...
            model_optimistic = Model(with_optimizer(Gurobi.Optimizer,gurobi_env))
            ...
            # Get the optimal objective value
            best_optimistic_assortment_revenue = getobjectivevalue(model_optimistic) + sum(ψ[i,i]*v[1][i] for i in S1_intersect_S2)
